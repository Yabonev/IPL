...
Token Tokenizer::NextToken()
{
	...
	if (c == '"')
	{
		...
		return Token{ TokenType::String, m_Line, IPLString(m_Code + start, m_Code + m_Current), 0. };
	}
	...
	if (c >= 'a' && c <= 'z')
	{
		...
		{
			return Token{ word->second, m_Line, key, 0. };
		}
	...
	if (IsValidIdentifierStartingChar(c))
	{
		...
		return Token{ TokenType::Identifier, m_Line, IPLString(m_Code + start, m_Code + m_Current), 0. };
	}
	// TODO: Error reporting
	return Token{ TokenType::Eof, m_Line, "", 0. };
}
